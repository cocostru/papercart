			...
			if ($final_shipping_method) {
				...
				foreach ($final_shipping_method as $shipping_method) {
					...
					$quote_data['mvdm_flat' . $shipping_method['courier_id']] = array(
						...
						'text'         => $this->currency->format($this->tax->calculate($shipping_method['shipping_rate'], $this->config->get('mvdm_flat_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
					);
					$i++;
				}
