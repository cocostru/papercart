		...
		if ($this->user->isLogged() && isset($this->request->get['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
           ...
           if (!$this->user->getVP()) {
				...
				if (!$this->user->IsAccountExpired()) {
					$this->response->redirect($this->url->link('common/vdi_dashboard', 'token=' . $this->session->data['token'], true));
				} else {
					$this->load->model('catalog/prostatctrl');
					$this->model_catalog_prostatctrl->DisabledAllProducts($this->user->getId());
