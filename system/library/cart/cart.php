...
class Cart {
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		...
		if (!$query->row['total']) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = (quantity + " . (float)$quantity . ") WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");
		}
	}
