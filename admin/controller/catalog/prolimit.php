	...
	private $error = array();
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/prolimit', 'token=' . $this->session->data['token'] . $url, true));
    	}
  	...
  	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/prolimit', 'token=' . $this->session->data['token'] . $url, true)
		);
		
		$data['delete'] = $this->url->link('catalog/prolimit/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['insert'] = $this->url->link('catalog/prolimit/insert', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {	
			...
			$data['product_limits'][] = array(
				...
				'edit'     			=> $this->url->link('catalog/prolimit/update', 'token=' . $this->session->data['token'] . '&product_limit_id=' . $result['product_limit_id'] . $url, true)
			);
    	}
		...
		$data['sort_package_name'] = $this->url->link('catalog/prolimit&token=' . $this->session->data['token'] . '&sort=package_name' . $url, true);
		$data['sort_product_limit'] = $this->url->link('catalog/prolimit&token=' . $this->session->data['token'] . '&sort=product_limit' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/prolimit&token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/prolimit', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('catalog/prolimit_list', $data));
  	}
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['product_limit_id'])) {
			$data['action'] = $this->url->link('catalog/prolimit/insert', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/prolimit/update', 'token=' . $this->session->data['token'] . '&product_limit_id=' . $this->request->get['product_limit_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/prolimit', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('catalog/prolimit_form', $data));
  	}
