	...
	private $error = array();
  	...
  	private function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/prostatctrl', 'token=' . $this->session->data['token'], true)
		);

		$data['update'] = $this->url->link('catalog/prostatctrl/update', 'token=' . $this->session->data['token'] . $url, true);
		...
		$pagination->url = $this->url->link('catalog/prostatctrl', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('catalog/prostatctrl_list', $data));
  	}
