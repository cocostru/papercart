	...
	private $error = array();
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/courier', 'token=' . $this->session->data['token'] . $url, true));
    	}
  	...
  	private function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/courier', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['insert'] = $this->url->link('catalog/courier/insert', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('catalog/courier/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['couriers'][] = array(
				...
				'edit'     			=> $this->url->link('catalog/courier/update', 'token=' . $this->session->data['token'] . '&courier_id=' . $result['courier_id'] . $url, true)
			);
    	}
		...
		$data['sort_courier_name'] = $this->url->link('catalog/courier&token=' . $this->session->data['token'] . '&sort=courier_name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/courier&token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/courier', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('catalog/courier_list', $data));
  	}
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['courier_id'])) {
			$data['action'] = $this->url->link('catalog/courier/insert', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/courier/update', 'token=' . $this->session->data['token'] . '&courier_id=' . $this->request->get['courier_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/courier', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('catalog/courier_form', $data));
  	}
