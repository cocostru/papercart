...
class ControllerDashboardVDIOrderStock extends Controller {
	public function index() {
		...
		foreach ($results as $result) {
			...
			$replace = array(
				$this->url->link('catalog/vdi_product/edit', 'token=' . $this->session->data['token'] . '&product_id=', true),
				$this->url->link('sale/vdi_order/info', 'token=' . $this->session->data['token'] . '&order_id=', true)
			);
		...
		return $this->load->view('dashboard/vdi_order_stock', $data);
	}
}
