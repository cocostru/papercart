...
class ControllerCommonVDIHeader extends Controller {
	public function index() {
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			$data['logged'] = '';
			$data['home'] = $this->url->link('common/vdi_dashboard', '', true);
		} else {
			$data['logged'] = true;
			...
			$data['processing_status'] = $this->url->link('sale/vdi_order', 'token=' . $this->session->data['token'] . '&filter_order_status=' . implode(',', $this->config->get('config_processing_status')), true);
			...
			$data['complete_status'] = $this->url->link('sale/vdi_order', 'token=' . $this->session->data['token'] . '&filter_order_status=' . implode(',', $this->config->get('config_complete_status')), true);
			...
			$data['product'] = $this->url->link('catalog/vdi_product', 'token=' . $this->session->data['token'] . '&filter_quantity=0', true);
			...
			$data['product_pending_approval'] = $this->url->link('catalog/vdi_product', 'token=' . $this->session->data['token'] . '&filter_status=5', true);
		...
		return $this->load->view('common/vdi_header', $data);
	}
}
