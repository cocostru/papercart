...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		$data['footer'] = $this->load->controller('common/footer');
			...
			$data['product_description'][1]['description'] = (int)$product_info['height'] . ' / ' . ($product_info['length'] > 0 ? number_format($product_info['length'], 1) : 0) . ' x ' . number_format($product_info['width'], 1) . ' (' . ($product_info['length'] > 0 ? number_format($this->length->convert($product_info['length'], $product_info['length_class_id'], 3), 1) : 0) . $this->length->getUnit(3) . ' x ' . ($product_info['width'] > 0 ? number_format($this->length->convert($product_info['width'], $product_info['length_class_id'], 3), 1) : 0) . $this->length->getUnit(3) . ') ' . ($this->model_catalog_manufacturer->getManufacturer($product_info['manufacturer_id']) ? $this->model_catalog_manufacturer->getManufacturer($product_info['manufacturer_id'])['name'] : '');
